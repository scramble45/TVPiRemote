      ...
      async.auto({
        ...
        func2: ["Func1", function (results, callback) {
          ...
          if (ampconfig.Power == 'ON') {
            ...
            var tvOff = execSh(["sleep 3 && irsend SEND_ONCE Benq850 OFF && sleep 2 && irsend SEND_ONCE Benq850 OFF"], true, // turn off projector
              function (err, stdout, stderr) {
                if (err || stderr) {
                  console.error("error: ", err)
                  console.error("stderr: ", stderr)
            ...
            var tvOn = execSh(["irsend SEND_ONCE Benq850 ON"], true, // turn on projector
              function (err, stdout, stderr) {
                if (err || stderr) {
                  console.error("error: ", err)
                  console.error("stderr: ", stderr)
...
io.on('connection', function (socket) {
  ...
  app.get('/mute', function (req, res) {
    res.send('Volume Muted...')
    get(mainZoneSendCmd + 'PutVolumeMute/on')
  ...
  app.get('/mister', function (req, res) {
    res.send('Source MiSTer toggled...')
    get(mainZoneSendCmd + 'PutZone_InputFunction%2FGAME')
    console.log('Source MiSTer was toggled from alexa')
  })
